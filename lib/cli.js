...
function setupCliObject () {
	...
	cliOptions
		...
		.option('-v, --verbose',
			'Whether or not we should be verbose with our output. (cannot be used with --quiet ' +
			'or --silent options)')
		.option('-q, --quiet',
			'Using this option will result in only the final outcome being displayed.')
		.option('-s, --silent',
			'Whether or not we should show anything on screen at all.')
		.option('-r, --recursive',
			'Whether or not we should recurse into local and external dependencies.')
		.option('-d, --dep-paths <paths>',
			'Colon separated list of paths to external dependencies. (default: "./components")')
		.option('-w, --warn-on-deps',
			'If provided, errors caused by external dependencies will only result in a warning ' +
...
exports.execute = function (cb) {
	...
	ensurePathsExist([].concat(checkPaths, depPaths, lookupPaths), function (error) {
		...
		// Load coponent data and perform onTheFly tests
		componentHint.loadPaths(checkPaths, null, function () {
