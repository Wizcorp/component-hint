...
exports.execute = function (args, cb) {
	...
	var depPaths = pathsArgToPathArray(argsObj.depPaths || ['./components']);
	...
	async.series([
		...
		function (callback) {
			...
			ensurePathsExist(depPaths, callback);
		},
		function (callback) {
			// Make sure all the lookup paths exist
		...
		async.forEachSeries(checkPaths, function (checkPath, callback) {
			componentHint.lintPath(checkPath, depPaths, lookupPaths, recursive, lintErrorsObj, callback);
		}, function () {
