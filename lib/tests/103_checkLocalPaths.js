	...
	async.eachSeries(componentPaths, function (lookupPath, callback) {
		...
		fs.exists(checkPath, function (exists) {
			if (exists) {
				...
				usedPaths.push(lookupPath);
			}

			return callback();
		});
	}, function () {
		return cb(resolvedPaths, usedPaths);
	});
}
...
exports.onTheFlyTest = function (componentData, componentHint, cb) {
	...
	getValidPaths(componentData, componentHint, function (validPaths) {
		...
		async.eachSeries(localDeps, function (depName, callback) {
			resolveDependencyPaths(depName, componentPath, validPaths, function (resolvedPaths, usedPaths) {
				// Remove all used paths from the unused paths array
				for (var i = 0; i < usedPaths.length; i += 1) {
					var usedPathI = unusedPaths.indexOf(usedPaths[i]);
					if (usedPathI >= 0) {
