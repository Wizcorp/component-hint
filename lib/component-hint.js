...
for (var testI = 0; testI < testFiles.length; testI += 1) {
	...
	if (testFile[0] === '.' || testFile.slice(-3) !== '.js') {
		continue;
	}
 ...
 * @returns {undefined}
 */
ComponentHint.prototype.loadComponentData = function (componentPath, extras, cb) {
	var self = this;
	var componentJson;
	...
	if (extras.isExternalDep && this.warnOnDeps) {
 ...
 * @returns {unresolved}
 */
ComponentHint.prototype.onTheFlyChecks = function (checkPath, extras, cb) {
	var self = this;
	extras = extras || {};
	...
	this.loadComponentData(absolutePath, extras, function (error, componentData) {
		if (error) {
		...
		async.eachSeries(Object.keys(onTheFlyTests).sort(), function (testName, callback) {
			...
			async.eachSeries(resolvedLocalPaths, function (depPath, callback) {
				self.onTheFlyChecks(depPath, extras, callback);
			}, function () {
				// Mark these and their deps as external deps from here on
				extras.isExternalDep = true;

				// Now recurse into external deps
				async.eachSeries(resolvedDepPaths, function (depPath, callback) {
					self.onTheFlyChecks(depPath, extras, callback);
				}, cb);
			});
		});
