...
var checkDependencies = require('./tests/checkDependencies.js').test;
 ...
 * @returns {undefined}
 */
exports.lintPath = function (checkPath, depPaths, lookupPaths, recursive, lintErrorsObj, cb) {
	...
	loadData(absolutePath, function (error, componentData) {
		...
		// Inject into data
		componentData.depPaths = depPaths;
		componentData.lookupPaths = lookupPaths;
		componentData.recursive = recursive;
		...
		async.series([
			function (callback) {
				...
				checkLocalPaths(absolutePath, componentData, function (error, lintErrors, resolvedDeps) {
					resolvedDepPaths.push.apply(resolvedDepPaths, resolvedDeps || []);
					return callback(null, lintErrors);
				});
			},
			function (callback) {
				// Lint dependencies
				checkDependencies(absolutePath, componentData, function (error, lintErrors, resolvedDeps) {
					resolvedDepPaths.push.apply(resolvedDepPaths, resolvedDeps || []);
					return callback(null, lintErrors);
				});
			}
			...
			if (lintErrors.length) {
				lintErrorsObj[absolutePath] = lintErrors;
			}
			...
			async.forEachSeries(resolvedDepPaths, function (depPath, callback) {
				exports.lintPath(depPath, depPaths, lookupPaths, recursive, lintErrorsObj, callback);
			}, cb);
		});
	});
};